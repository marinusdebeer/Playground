...
INITIAL_LEARNING_RATE = 0.001 #0.00025 #0.002 #0.01 for BN
LEARNING_RATE_DECAY = 0.3   #0.05 for 5_000, 0.1 for 10_000
MIN_LEARNING_RATE = 0.0005
EPSILON_START = 1.0
EPSILON_DECAY = 0.06          #0.04 for 5_000, 0.01 for 20_000
EPSILON_MIN = 0.10
SEED=42
...
class DQN(tf.keras.Model):
    def __init__(self):
        ...
        if DUELING_DQN:
            self.dense1_adv = Dense(512, activation='leaky_relu', kernel_initializer=HeNormal())
            self.dense2_adv = Dense(NUM_ACTIONS, kernel_initializer=HeNormal())
            self.dense1_val = Dense(512, activation='leaky_relu', kernel_initializer=HeNormal())
            self.dense2_val = Dense(1, kernel_initializer=HeNormal())
        else:
            self.dense1 = Dense(512, activation='leaky_relu', kernel_initializer=HeNormal())
...
MODEL = {MODEL}\n\n"""
    ...
    def compute_gradients_and_heatmap(self, model, input_frames, action):
        input_frames = tf.convert_to_tensor(input_frames, dtype=tf.float32)
        with tf.GradientTape() as tape:
            tape.watch(input_frames)
            q_values = model(input_frames)
            q_action = q_values[:, action]

        # Gradients of Q-value with respect to the input frames
        gradients = tape.gradient(q_action, input_frames)

        # Process gradients for each frame
        heatmaps = []
        for i in range(gradients.shape[1]):
            gradient = gradients[0, i, :, :, :]
            heatmap = tf.reduce_sum(tf.abs(gradient), axis=-1)
            normalized_heatmap = (heatmap - tf.reduce_min(heatmap)) / (tf.reduce_max(heatmap) - tf.reduce_min(heatmap))
            heatmaps.append(normalized_heatmap)
    ...
    def learn(self, episode, steps, last10rewards):
        ...
        log_start = time.time()
        # sample_frames = states[0:1]
        # sample_action = actions[0]
        # heatmaps = self.compute_gradients_and_heatmap(self.q_network, sample_frames, sample_action)
        if steps > 10_000 and steps % (TRAINING_FREQ*100) == 0:
            with file_writer.as_default():
                ...
                # for i, (grad, var) in enumerate(zip(gradients, self.q_network.trainable_variables)):
                #     if grad is not None:
                #         tf.summary.scalar(f'grad_norm/{var.name}', tf.norm(grad), step=steps)

                # overall_norm = tf.norm([tf.norm(grad) for grad in gradients if grad is not None])
                # tf.summary.scalar('grad_norm/overall', overall_norm, step=steps)
                # for i, heatmap in enumerate(heatmaps):
                #     tf.summary.image(f"Gradient Heatmap Frame {i+1}", tf.expand_dims(heatmap, -1), step=steps)
                # print(f"plotting: {overall_norm}")
                file_writer.flush()
    ...
    def preprocess_state(self, state):
        ...
        _, processed_observe = cv2.threshold(processed_observe, 0, 1, cv2.THRESH_BINARY)
    ...
    def train(self):
        ...
        for episode in range(1, NUM_EPISODES + 1):
            ...
            # elif episode > 7_500:
            #     self.epsilon = decay(0.30, EPSILON_DECAY, episode - 7_500, 2_500, EPSILON_MIN)
            # elif episode > 5_000:
            #     self.epsilon = decay(0.30, EPSILON_DECAY, episode - 5_000, 2_500, EPSILON_MIN)
            # elif episode > 2_500:
            #     self.epsilon = decay(0.30, EPSILON_DECAY, episode - 2_500, 2_500, EPSILON_MIN)
            else:
                self.epsilon = decay(EPSILON_START, EPSILON_DECAY, episode, 5_000, EPSILON_MIN)
...
if __name__ == "__main__":
    ...
    # Notes
    # Let epsilon go for 5000 episodes
    # Play the game and save all the states, rewards, etc. then let the model learn that until it completely overfits
    # and can play it the same then use the models weights as the initialization weights
    # Add dropout if it is overfitting but just test it anyways
    # Save model in end
    # lr=0.001 to 0.00025
    # Put learing rate in a file that way i can change it during a run
