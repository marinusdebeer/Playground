...
DISCOUNT = 0.99
epsilon = 1
EPSILON_DECAY = 0.9985 ## 0.9975 99975
MIN_EPSILON = 0.1
...
class CarEnv:
    ...
    def reset(self):
        self.collision_hist = []
        self.actor_list = []
        self.transform = carla.Transform(carla.Location(x=-11.1, y=138.5, z=0.1), carla.Rotation(yaw=180))
...
class DQNAgent:
    def __init__(self):
        # self.model = load_model('models/Xception.11.model')
        # self.target_model = load_model('models/Xception.11.model')
...
if __name__ == '__main__':
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        #try:
            ...
            if not episode % AGGREGATE_STATS_EVERY or episode == 1:
                ...
                if average_reward >= MIN_REWARD or not episode % (AGGREGATE_STATS_EVERY*5):
    ...
    trainer_thread.join()
    # agent.model.save(f'models/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.model')
    agent.model.save(f'models/{MODEL_NAME}_{average_reward}_{min_reward}_{max_reward}.{episode}_trail_{TRIAL}.model')
